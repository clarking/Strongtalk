...
protected:
    ...
    std::size_t *createBitString( std::size_t l ) {
        ...
        set_words(reinterpret_cast<int32_t *>(bs), blen, 0 );
        return bs;
    }
    ...
    std::size_t *copyBitString( std::size_t len ) {
        ...
        copy_words(reinterpret_cast<int32_t *>(_bits), reinterpret_cast<int32_t *>(bs), blength );
        if ( blength < blen ) {
            set_words(reinterpret_cast<int32_t *>(bs + blength), blen - blength, 0 );
        }
...
public:
    ...
    void clear() {
        set_words(reinterpret_cast<int32_t *>(_bits), bitsLength(length ), 0 );
    }
