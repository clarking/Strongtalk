...
ObjectArrayOop MethodOopDescriptor::fileout_body() {
	...
	do {
		if (ByteCodes::send_type(c.code()) not_eq ByteCodes::SendType::NO_SEND) {
			...
			out.put_byte(static_cast<std::int32_t>(original));
			out.align(c.hp() + 1);
			if (c.code() == ByteCodes::Code::prim_call or c.code() == ByteCodes::Code::primitive_call_failure or c.code() == ByteCodes::Code::primitive_call_self or
			    c.code() == ByteCodes::Code::primitive_call_self_failure) {
