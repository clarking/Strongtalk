	...
	if (is_message_send()) {
		switch (argumentsType()) {
			...
			case ByteCodes::ArgumentSpec::recv_n_args:
				return pop_result() ? (restore_value ? Interpreter::deoptimized_return_from_send_with_receiver_pop_restore() : Interpreter::deoptimized_return_from_send_with_receiver_pop())
				                    : (restore_value ? Interpreter::deoptimized_return_from_send_with_receiver_restore() : Interpreter::deoptimized_return_from_send_with_receiver());
			case ByteCodes::ArgumentSpec::args_only:
				return pop_result() ? (restore_value ? Interpreter::deoptimized_return_from_send_without_receiver_pop_restore() : Interpreter::deoptimized_return_from_send_without_receiver_pop())
	...
	if (is_primitive_call()) {
		switch (code()) {
			...
			case ByteCodes::Code::primitive_call_self_lookup:
				return restore_value ? Interpreter::deoptimized_return_from_primitive_call_without_failure_block_restore()
...
void CodeIterator::customize_class_var_code(KlassOop to_klass) {
	st_assert(code() == ByteCodes::Code::push_classVar_name or code() == ByteCodes::Code::store_classVar_pop_name or code() == ByteCodes::Code::store_classVar_name,
	          "must be class variable byte code");
...
void CodeIterator::customize_inst_var_code(KlassOop to_klass) {
	st_assert(code() == ByteCodes::Code::push_instVar_name or code() == ByteCodes::Code::store_instVar_pop_name or code() == ByteCodes::Code::store_instVar_name or
	          code() == ByteCodes::Code::return_instVar_name, "must be instance variable byte code");
...
void CodeIterator::uncustomize_inst_var_code(KlassOop from_klass) {
	st_assert(code() == ByteCodes::Code::push_instVar or code() == ByteCodes::Code::store_instVar_pop or code() == ByteCodes::Code::store_instVar or code() == ByteCodes::Code::return_instVar,
	          "must be instance variable byte code");
