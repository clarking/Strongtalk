...
struct KlassOopDescriptor *
ClassChange::new_class_from(KlassOopDescriptor *old_klass, KlassOopDescriptor *new_super_klass, struct MixinOopDescriptor *new_mixin, Klass::Format new_format, struct MixinOopDescriptor *old_mixin) {
...
std::int32_t ClassChange::compute_needed_schema_change() {
	...
	if (new_super()->is_klass() and not(new_super()->klass_part()->has_same_layout_as(old_klass()->klass_part()->superKlass()) and
