...
template<
		typename FIRST
>
void write_debug_output(std::ostream &out, FIRST const &f) {
	out << f;
}
...
struct tracer {
	...
	tracer(std::ostream &out, char const *file, std::int32_t line) : _out(out) {
		out << file << ":" << line << ": ";
	}
	...
	void write(FIRST const &f, REST const &... rest) {
