...
constexpr auto nthBit(const auto n) {
	return OneBitSet << (n);
}

constexpr auto maskBits(const auto x, const auto m) {
	return x & m;
}

constexpr auto addBits(const auto x, const auto m) {
	return x | m;
}

constexpr auto subBits(const auto x, const auto m) {
	return x & ~m;
}

constexpr auto setBits(auto &x, const auto m) {
	return x |= m;
}

constexpr auto clearBits(auto &x, const auto m) {
	return x &= ~m;
}

constexpr auto addNthBit(const auto x, const auto n) {
	return addBits(x, nthBit(n));
}

constexpr auto subNthBit(const auto x, const auto n) {
	return subBits(x, nthBit(n));
}

constexpr auto setNthBit(auto x, const auto n) {
	return setBits(x, nthBit(n));
}

constexpr auto clearNthBit(auto &x, const auto n) {
...
constexpr auto anyBitSet(auto x, const auto m) {
	return maskBits(x, m) not_eq NoBitsSet;
}

constexpr auto isBitSet(auto x, const auto n) {
	return anyBitSet(x, nthBit(n));
}

constexpr auto nthMask(const auto n) {
	return (n == BITS_PER_WORD ? AllBitsSet : (nthBit(n) - OneBitSet));
}


// -----------------------------------------------------------------------------

constexpr auto lowerBits(auto x, const auto n) {
	return maskBits(x, nthMask(n));
}

constexpr auto roundMask(auto x, const auto m) {
	return (x + m) & ~m;
}

constexpr auto roundTo(auto x, const auto v) {
	return roundMask(x, v - OneBitSet);
}

constexpr auto roundBits(auto x, const auto n) {
	return roundMask(x, nthMask(n));
}
...
constexpr auto clearTag(auto Oop) {
	return reinterpret_cast<std::int32_t>( Oop ) & ~TAG_MASK;
}
